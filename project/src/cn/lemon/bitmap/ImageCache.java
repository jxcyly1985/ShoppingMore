...
public class ImageCache {
	...
	private void init(ImageCacheParams cacheParams) {
		...
		if (mCacheParams.mMemoryCacheEnabled) {
			...
			DebugUtil.debug(TAG, "Memory cache created (size = "
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache == null || mDiskLruCache.isClosed()) {
				...
				if (null != diskCacheDir) {
					DebugUtil.debug(TAG,
							"file path is:" + diskCacheDir.getPath());
						...
						if (getUsableSpace(diskCacheDir) > mCacheParams.mDiskCacheSize) {
							try {
								mDiskLruCache = DiskLruCache.open(diskCacheDir,
										1, 1, mCacheParams.mDiskCacheSize);

								DebugUtil.debug(TAG, "Disk cache initialized");

							} catch (final IOException e) {
								mCacheParams.mDiskCacheDir = null;
								DebugUtil.error(TAG, "initDiskCache - " + e);
							}
						}
					}
				}
				...
				try {
					...
					if (snapshot == null) {
						...
						if (editor != null) {
							...
							bitmap.compress(mCacheParams.mCompressFormat,
									mCacheParams.mCompressQuality, out);
							editor.commit();
							out.close();
						}
					} else {
					...
					DebugUtil.error(TAG, "addBitmapToCache - " + e);
				} catch (Exception e) {
					DebugUtil.error(TAG, "addBitmapToCache - " + e);
				} finally {
					try {
	...
	public void addBitmapToCache(String data, Bitmap bitmap, boolean mIsAddDisk) {
		if (mIsAddDisk) {
			// Add to memory cache and disk
	...
	public Bitmap getBitmapFromMemCache(String data) {
		if (mMemoryCache != null) {
			...
			if (memBitmap != null) {
				...
				DebugUtil.debug(TAG, "Memory cache hit");
			...
			if (mDiskLruCache != null) {
				...
				try {
					...
					if (snapshot != null) {
						...
						DebugUtil.debug(TAG, "Disk cache hit");
					...
					DebugUtil.error(TAG, "getBitmapFromDiskCache - " + e);
				} finally {
					try {
						if (inputStream != null) {
							inputStream.close();
	...
	public void clearCache() {
		if (mMemoryCache != null) {
			...
			DebugUtil.debug(TAG, "Memory cache cleared");
		}
		...
		synchronized (mDiskCacheLock) {
			...
			if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
				try {
					...
					DebugUtil.debug(TAG, "Disk cache cleared");

				} catch (IOException e) {
					DebugUtil.error(TAG, "clearCache - " + e);
				}
				mDiskLruCache = null;
				initDiskCache();
			}
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null) {
				try {
					...
					DebugUtil.debug(TAG, "Disk cache flushed");
			...
			if (mDiskLruCache != null) {
				try {
					if (!mDiskLruCache.isClosed()) {
						...
						DebugUtil.debug(TAG, "Disk cache closed");
					}
				} catch (IOException e) {
					DebugUtil.error(TAG, "close - " + e);
				}
			}
		}
	}
	...
	public static class ImageCacheParams {
		...
		public boolean mClearDiskCacheOnStart = DEFAULT_CLEAR_DISK_CACHE_ON_START;
		...
		public ImageCacheParams(Context context, String uniqueName) {
			mDiskCacheDir = getDiskCacheDir(context, uniqueName);
		}
		...
		public void setMemCacheSizePercent(Context context, float percent) {
			if (percent < 0.05f || percent > 0.8f) {
				throw new IllegalArgumentException(
						"setmMemCacheSizePercent - percent must be "
								+ "between 0.05 and 0.8 (inclusive)");
			}
			mMemCacheSize = Math.round(percent * getMemoryClass(context) * 1024
					* 1024);
		}
	...
	public static File getExternalCacheDir(Context context) {
		/*
		 * if (Utils.hasFroyo()) { return context.getExternalCacheDir(); }
		 */
		...
		String externalPath = Environment.getExternalStorageDirectory()
				.getAbsolutePath();
		String mPath = externalPath + cacheDir;

		DebugUtil.debug(TAG, "getDiskCacheDir<<<>>> " + mPath);
	...
	public static RetainFragment findOrCreateRetainFragment(FragmentManager fm) {
		...
		DebugUtil.debug(TAG, "findOrCreateRetainFragment=" + mRetainFragment);
		...
		if (mRetainFragment == null) {
			...
			// cause commit take effect immediately
